...
etfdax_ma_notc_type_tab <- etfdax_ma_notc_analysis$dat_type_rank %>% 
  select(-id) %>%
  ...
  kbl(., "latex", caption = "MA Strategy results -- ETFDAX", 
...
```{r, etfdax-ma-score, message=FALSE, warning=FALSE, fig.align = "center", out.width = "50%", fig.cap = "[MA, -tc] ETFDAX -- IR score by value of the long-term lag.", fig.ncol = 2, fig.nrow = 2, fig.subcap = c("For effective strategies (IR > 0)", "For effective strategies (IR > 0), with band $\\geq$ 0.04", "For the whole sample", "For the whole sample, with band $\\geq$ 0.04")}


etfdax_ma_notc_analysis$plot_lag_IR
etfdax_ma_notc_analysis$plot_lag_IRb
etfdax_ma_notc_analysis$plot_lag_IRws
etfdax_ma_notc_analysis$plot_lag_IRwsb
...
```{r, etfdax-ma-heatmaps, message=FALSE, fig.cap = "[MA, -tc] ETFDAX -- IR by lag length and band value.",  fig.dim=c(8,3), fig.ncol = 1, fig.align='center', fig.subcap = c("Information Ratio", "Signals per strategy")}
...
# 
etfdax_ma_notc_analysis$plot_heat_sma
etfdax_ma_notc_analysis$plot_heat_smas
...
```{r, etfdax-ma-drawdowns-tab, echo=FALSE}


etfdax_ma_notc_drawdowns_tab <- etfdax_ma_notc_results$drawdowns_data %>% 
  mutate(across(everything(),
                ~ case_when(is.na(.) ~ cell_spec(., "latex", color = "lightgray"),
  ...
  footnote(general = "This table provides a selection of performance measures for the two best, two worst and the middle standard MA strategy on ETFDAX, in comparison with the BH benchmark.",
...
```{r, etfdax-ma-drawdowns-fig, out.width = "100%", fig.cap = "[MA, -tc] ETFDAX -- Drawdowns for two top (blue and orange), two bottom (gray) and the middle (green) strategy, vs drawdowns for the BH benchmark (red).", fig.align = "center"}


knitr::include_graphics("data/results/static/ETFDAX_ma_notc_drawdowns.png")
