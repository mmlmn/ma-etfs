...
etfsp500_ma_notc_type_tab <- etfsp500_ma_notc_analysis$dat_type_rank %>% 
  select(-id) %>%
  ...
  kbl(., "latex", caption = "MA strategy results -- ETFSP500", 
  ...
  footnote(general = "In this table, we aggregate the standard MA strategy performance and parameter data for ETFSP500 across the three MA types. We break down the results into three subgroups: the first representing the data for strategies with positive IR (i.e. beating the BH strategy); the second and third subgroup characterising the top 25% and the bottom 25% of strategies, respectively.",
           number = c("All performance measures use standard formulation defined in Appendix A; ", "The performance and parameter statistics are calculated with respect to the subgroup size reported in column «N»; ", "Column «Share (%)» provides a subgroup's share in the whole sample (1170); ", "Transaction costs are not included."), 
           general_title = "",
           footnote_as_chunk = F,
           threeparttable = T,
           fixed_small_size = T,
...
```{r, etfsp500-ma-score, message=FALSE, warning=FALSE, fig.align = "center", out.width = "50%", fig.cap = "[MA, -tc] ETFSP500 -- IR score by value of the long-term lag.", fig.ncol = 2, fig.nrow = 2, fig.subcap = c("For effective strategies (IR > 0)", "For effective strategies (IR > 0), with band $\\geq$ 0.04", "For the whole sample", "For the whole sample, with band $\\geq$ 0.04")}


etfsp500_ma_notc_analysis$plot_lag_IR
etfsp500_ma_notc_analysis$plot_lag_IRb
etfsp500_ma_notc_analysis$plot_lag_IRws
etfsp500_ma_notc_analysis$plot_lag_IRwsb
...
```{r, etfsp500-ma-heatmaps, message=FALSE, fig.cap = "[MA, -tc] ETFSP500 -- IR by lag length and band value.",  fig.dim=c(8,3), fig.ncol = 1, fig.align='center', fig.subcap = c("Information Ratio", "Signals per strategy")}
...
# 
etfsp500_ma_notc_analysis$plot_heat_sma
etfsp500_ma_notc_analysis$plot_heat_smas
...
```{r, etfsp500-ma-drawdowns-tab, echo=FALSE}


etfsp500_ma_notc_drawdowns_tab <- etfsp500_ma_notc_results$drawdowns_data %>% 
  mutate(across(everything(),
                ~ case_when(is.na(.) ~ cell_spec(., "latex", color = "lightgray"),
  ...
  kbl(., "latex", caption = "Select strategy results -- ETFSP500", 
  ...
  footnote(general = "This table provides a selection of performance measures for the two best, two worst and the middle standard MA strategy on ETFSP500, in comparison with the BH benchmark.",
           number = c("IR values, as per Appendix A, refer to a strategy's result with respect to the BH benchmark; ", "SR shows the performance against a risk-free return for each strategy and the BH benchmark; ", "Colours in column «ID» respond to line colours in the following drawdown plot; ", "Transaction costs are not included."), 
           general_title = "",
           footnote_as_chunk = F,
           threeparttable = T,
           fixed_small_size = T,
...
etfsp500_ma_notc_drawdowns_tab
...
```{r, etfsp500-ma-drawdowns-fig, out.width = "100%", fig.cap = "[MA, -tc] ETFSP500 -- Drawdowns for two top (blue and orange), two bottom (gray) and the middle (green) strategy, vs drawdowns for the BH benchmark (red).", fig.align = "center"}


knitr::include_graphics("data/results/static/ETFSP500_ma_notc_drawdowns.png")
