...
etfsp500_ma_tc_type_tab <- etfsp500_ma_tc_analysis$dat_type_rank %>% 
  select(-id) %>%
...
```{r, etfsp500-ma-score, message=FALSE, warning=FALSE, fig.align = "center", out.width = "50%", fig.cap = "[MA] ETFSP500 -- IR score by value of the long-term lag.", fig.ncol = 2, fig.nrow = 2, fig.subcap = c("For the whole sample", "For the whole sample, with band $\\geq$ 0.04")}


etfsp500_ma_tc_analysis$plot_lag_IRws
etfsp500_ma_tc_analysis$plot_lag_IRwsb
...
```{r, etfsp500-ma-heatmaps, message=FALSE, fig.cap = "[MA] ETFSP500 -- IR by lag length and band value.",  fig.dim=c(8,3), fig.ncol = 1, fig.align='center', fig.subcap = c("Information Ratio", "Signals per strategy")}
...
# 
etfsp500_ma_tc_analysis$plot_heat_sma
etfsp500_ma_tc_analysis$plot_heat_smas
...
etfsp500_ma_tc_drawdowns_tab <- etfsp500_ma_tc_results$drawdowns_data %>% 
  mutate(across(everything(),
                ~ case_when(is.na(.) ~ cell_spec(., "latex", color = "lightgray"),
...
```{r, etfsp500-ma-drawdowns-fig, out.width = "100%", fig.cap = "[MA] ETFSP500 -- Drawdowns for two top (blue and orange), two bottom (gray) and the middle (green) strategy, vs drawdowns for the BH benchmark (red).", fig.align = "center"}


knitr::include_graphics("data/results/static/ETFSP500_ma_tc_drawdowns.png")
